...
# 银行开户逻辑
def bank_adduser(account, username, password, country, province, street, door, money, reagisterDate, bank_name):
...
def outputMoney():
    ...
    if withdrawals == 1:
        ...
        # 取款后金额
        sql8 = "update 银行 set money=money-%s where account=%s"
        param8 = [output_money,account_get]
        update(sql8, param8)
        # 查询余额
        sql10="select money from 银行 where account=%s"
        param10=[account_get]
...
def bank_transfer(out_account, in_account, out_money):
    # 判断转出的账号是否存在
    sqla="select * from 银行 where account=%s"
    parama=[out_account]
    datea=select(sqla,parama,mode="one")[0]
    # 判断转入的账号是否存在
    sqlb="select * from 银行 where account=%s"
    paramb=[in_account]
...
def transfer():
    ...
    if Transfer == 1:
        ...
        update(sqle,parame)
        # 查询转出账号的余额
        sqlqq="select money from 银行 where account=%s"
        paramqq=[out_account]
        dateqq=select(sqlqq,paramqq,mode="one")[0]
        print("转账成功！")
        print("转出账号余额为：", dateqq)
        # 转账后转入账号余额
        sqlww="update 银行 set money=money+%s where account=%s"
...
def search():
    ...
    # 查询账号的密码是否正确
    sqlh="select password from 银行 where account=%s"
    paramh=[account_src]
